...
Object.assign(World.prototype, {
  ...
  addToMap(mo){
    if(!mo) return;
    const im = mo.img, ok = im && im.complete && im.naturalWidth > 0 && !im._broken;
    if(!ok){
      if(im?._broken && !mo._warned){ console.warn('Broken image:', mo.constructor?.name, im?.src); mo._warned = true; }
    ...
    const c=this.ctx, flip=(mo.facing===-1)||mo.otherDirection===true;
    c.save();
    if(mo.state!=='dead') mo.drawGroundShadow?.(c, this.groundY, { alpha:.12, ryFactor:.1 });
    if(typeof mo.alpha==='number') c.globalAlpha = mo.alpha;
    if(flip){ c.translate(mo.x+mo.width,mo.y); c.scale(-1,1); c.drawImage(im,0,0,mo.width,mo.height); }
    else { c.drawImage(im,mo.x,mo.y,mo.width,mo.height); }
    c.restore();
  }
});
