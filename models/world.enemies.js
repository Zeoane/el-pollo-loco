...
Object.assign(World.prototype, {
  ...
  spawnSmallChickens(){
    const c = this.cfg.enemies || {}, n = c.smallCount || 4;
    let x = (this.character.x||0) + 500;
    for (let i=0;i<n;i++){
      const h = new SmallChicken().setGround?.(this.groundY).placeOnGround?.();
      h.speed = (c.smallSpeedMin ?? 1.2) + Math.random()*((c.smallSpeedMax ?? 2.0)-(c.smallSpeedMin ?? 1.2));
      h.x = x; this.opponents.push(h);
      x += (c.smallGapMin ?? 220) + Math.random()*((c.smallGapMax ?? 460)-(c.smallGapMin ?? 220));
    }
  },

  spawnRegularChickens(){
    const c = this.cfg.enemies || {}, n = c.regCount || 3;
...
spawnBoss(/* ... */){
  ...
  updateOpponents(dtMs){
    ...
    this.opponents.forEach(o => {
      ...
      if (o.updateBoss){ o.updateBoss(this, dtMs); return; }
      o.x -= o.speed ?? 0;
