...
Object.assign(World.prototype, {
  ...
  updateThrow(dtMs){
    const KB = (window.keyboard || this.keyboard || {});
    const fire = KB.F || KB.G || KB.THROW, ts = this.throwState;
    if (fire && !ts.charging){ ts.charging=true; ts.holdMs=0; }
    if (fire && ts.charging)  ts.holdMs = Math.min(ts.holdMs + dtMs, ts.maxMs);
    if (!fire && ts.charging){
      ts.charging=false;
      if ((this.inventory.bottles||0)>0){
        const pow = 1 + (ts.holdMs/ts.maxMs)*1.5, dir = (this.character.facing===-1)?-1:1;
        this.projectiles.push(new Projectile(this.character.x+this.character.width/2, this.character.y+20, dir, {speedMul:pow}));
        this.inventory.bottles--; SFX.play?.('bottle_throw',{vol:.8});
      }
    }
  },
...
if (window.DEBUG_HEAL) console.log('HEAL STATE',{
  healKey: !!this.keyboard?.HEAL,
  cd: this._healCdMs,
  hp: this.character?.hp, max: this.character?.hpMax,
  ...
  if (!KB.HEAL) return;
  if (this.character?.state === 'dead' || (this.character?.hp??0) <= 0) return;
  if (this._healCdMs > 0) return;

  const max = this.character.hpMax ?? 100, hp = this.character.hp ?? max;
  ...
  this.inventory.coins -= cost; this.character.hp = Math.min(max, hp + gain);
  SFX.play?.('bottle_pick', { vol:.8 }); this._healCdMs = this.healCfg.cdMs || 1000;

  if (window.DEBUG_HEAL) console.log('HEAL OK',{hp:this.character.hp, coins:this.inventory.coins});
}
});
